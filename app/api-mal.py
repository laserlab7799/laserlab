...
def elections_state_ru(
    ...
    # ----------------------------------------------------------------
    # HARDENING TEST HOOKS (always-on)
    #   1) NJ Governor (officeId="G") => return intentionally MALFORMED XML
    #   2) VA Attorney General (officeId="A") => return HTML error page (500)
    # ----------------------------------------------------------------
    if usps == "NJ" and officeId == "G":
        # Mismatched closing tag + missing root terminator => malformed XML
        malformed = (
            f'<ElectionResults Date="{date}" StatePostal="{usps}" Office="{officeId}">\n'
            f'  <ReportingUnit Name="Statewide" FIPS="{USPS_TO_STATE_FIPS.get(usps, "34")}">\n'
            f'    <Candidate First="Jack" Last="Ciattarelli" Party="REP" VoteCount="12345"/>\n'
            f'    <Candidate First="Mikie" Last="Sherrill" Party="DEM" VoteCount="23456"/>\n'
            f'  </ReportingUnitX>\n'  # <-- intentionally wrong closing tag
            # (no closing </ElectionResults>)
        )
        return Response(content=malformed, media_type="application/xml", status_code=200)

    if usps == "VA" and officeId == "A":
        html = (
            "<!doctype html><html><head><title>500 Internal Server Error</title></head>"
            "<body><h1>Internal Server Error</h1>"
            "<p>Simulated backend failure for Attorney General (VA).</p>"
            "<hr><pre>trace: upstream aggregator timeout</pre></body></html>"
        )
