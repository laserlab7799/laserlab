...
FULL_DATA_URL = os.getenv(
    "FULL_DATA_URL",
    f"https://api.ap.org/v3/elections/2024-11-05?level={LEVEL_PARAM}"
)
...
def _parse_full_dataset(xml_text: str):
    ...
    for race in root.findall(".//Race"):
        ...
        if office == "H":
            ...
            # ---- Statewide offices (P, S, G, A, M ...): make counties
            st["race_call"] = {
                "status": state_status,
                "winner": winner_payload,
                "source": "api"
            ...
            for ru in rus:
                ...
                # For NYC Mayor, keep only 5 borough FIPS
...
def _hub_loop():
    log("Hub (single-call) poller starting..." if HUB_MODE else "Hub disabled (serve-only).")
    if not HUB_MODE:
        return
    ...
    while True:
        ...
        try:
            ...
            log(f"Unhandled in full-dataset fetch: {e}", "WARN")
        if ok:
            log("Full-dataset refresh complete.")
        else:
            log("Full-dataset refresh failed; will retry on next tick.", "WARN")
        time.sleep(max(5.0, POLL_INTERVAL_SECONDS))
...
def cache_ru():
    ...
    if office == "H" and (race_type or "").upper() == "G":
        with _cache_lock:
            la_l_bucket = _cache["cache_by_combo"].get(_combo_key("H", "L"), {})
            la_state = (la_l_bucket.get("states") or {}).get("LA")
        if la_state and "LA" not in states:
            # Don’t mutate the shared dict—copy and graft
            states = dict(states)
            states["LA"] = la_state
