...
def main():
    ...
    # Robust inference:
    # - House race folder: .../win_prob/H/CA/45
    # - Statewide folder:  .../win_prob/P/CA
    district = None

    if cwd.name.isdigit():  # e.g. "45"
        district = cwd.name.zfill(2)       # "45" -> "45", "5" -> "05"
        state = cwd.parent.name.upper()    # "CA"
        office = cwd.parent.parent.name.upper()  # "H"
    else:
        # e.g. cwd == ".../win_prob/P/CA"
        state = cwd.name.upper()           # "CA"
        office = cwd.parent.name.upper()   # "P"
    ...
    for ts in targets:
        ...
        # statewide % in (needed for box stats + gates)
        try:
            x = statewide_percent_in(snap)
        except RuntimeError:
            continue

        # --- compute MC median margin% NOW (so raw leader can follow it) ---
        ...
        mc_med = float(bx.get("med", np.nan))

        # ---- RAW leader FROM MC MEDIAN SIGN ----
        # mc_med is (Harris − Trump)
        # rule: mc_med > 0 => Harris/blue; otherwise => Trump/red (includes mc_med == 0 and non-finite)
        if np.isfinite(mc_med) and (mc_med > 0):
            raw_leader_name = "Harris"
        else:
            raw_leader_name = "Trump"

        leader_raw = harris_raw if raw_leader_name == "Harris" else trump_raw
        ...
        leader_adj = max(50.0, leader_raw - penalty_pts)
        trailer_adj = 100.0 - leader_adj

        # ---- ADJUSTED leader ----
        if raw_leader_name == "Trump":
            trump_adj  = leader_adj
            harris_adj = trailer_adj
            leader_name = "Trump"
        else:
            harris_adj = leader_adj
            trump_adj  = trailer_adj
            leader_name = "Harris"
        ...
        # ---------- EARLY COUNT HARD GATE ----------
        # If %in <= 5%, apply a -50pt penalty to the leader
        # Floor remains 50–50 as usual

        early_gate_penalty_pts = 0.0

        if x <= 5.0:
            if trump_adj >= harris_adj:
                before = trump_adj
                trump_adj = max(50.0, trump_adj - 50.0)
                harris_adj = 100.0 - trump_adj
                early_gate_penalty_pts = before - trump_adj
            else:
                before = harris_adj
                harris_adj = max(50.0, harris_adj - 50.0)
                trump_adj = 100.0 - harris_adj
                early_gate_penalty_pts = before - harris_adj
                
        # ---------- RACE-SPECIFIC HARD GATE (H/CA/45) ----------
        # If this is House CA-45 and %in < 75%, apply a -50pt penalty to the leader
        # Floor remains 50–50 as usual

        hc45_gate_penalty_pts = 0.0

        if office == "H" and state.upper() == "CA" and district == "45" and x < 75.0:
            if trump_adj >= harris_adj:
                before = trump_adj
                trump_adj = max(50.0, trump_adj - 50.0)
                harris_adj = 100.0 - trump_adj
                hc45_gate_penalty_pts = before - trump_adj
            else:
                before = harris_adj
                harris_adj = max(50.0, harris_adj - 50.0)
                trump_adj = 100.0 - harris_adj
                hc45_gate_penalty_pts = before - harris_adj


        # estimate final statewide total votes for this snapshot
        # (final_total_est, bx, mc_med, raw_leader_name, leader_raw were computed earlier)
        median_gate_penalty_pts = 0.0
        pa_gate_penalty_pts = 0.0
        gate_penalty = 0.0


        # ---------- Median-based gate ----------
        if np.isfinite(mc_med):
            ...
            # ---- Apply median leader-only penalty ----
            if gate_penalty > 0:
                if trump_adj >= harris_adj:
                    before = trump_adj
                    trump_adj = max(50.0, trump_adj - gate_penalty)
                    harris_adj = 100.0 - trump_adj
                    median_gate_penalty_pts = before - trump_adj
                else:
                    before = harris_adj
                    harris_adj = max(50.0, harris_adj - gate_penalty)
                    trump_adj = 100.0 - harris_adj
                    median_gate_penalty_pts = before - harris_adj


        # ---------- PA structural gate (ONLY before 75% in) ----------
        if x < 75.0 and args.state.upper() == "PA" and harris_adj > trump_adj:
            before = harris_adj
            harris_adj = max(50.0, harris_adj - 26.0)
            trump_adj = 100.0 - harris_adj
            pa_gate_penalty_pts = before - harris_adj



        else:
            # keep a stable value for CSV even if rule doesn't trigger
            mc_med = mc_med
        ...
        rows.append({
            ...
            "mc_median_margin_pct": mc_med,
            "median_gate_penalty_points_applied": median_gate_penalty_pts,
    ...
    with open(box_json_path, "w", encoding="utf-8") as f:
        json.dump(clean_nans(box_json), f, ensure_ascii=False, indent=2)
