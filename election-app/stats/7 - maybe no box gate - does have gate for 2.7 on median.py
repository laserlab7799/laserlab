...
def main():
    ...
    for ts in targets:
        ...
        # ---- NEW: median + early-%in gate (applies on top of all prior penalties) ----
        # If MC median FINAL margin% is within (-2.7, +2.7) and statewide %in <= 75,
        # reduce the *higher* adjusted win prob by 15 points (floor 50/50).
        ...
        # Adjusted win prob = base penalty result (NO q1/q3 box gate)
        leader_adj = leader_adj_base
        trailer_adj = 100.0 - leader_adj
        trump_adj  = leader_adj if leader_name == "Trump" else trailer_adj
        harris_adj = leader_adj if leader_name == "Harris" else trailer_adj

        # ---- median + early-%in gate (ON TOP of all prior penalties) ----
        # If MC median FINAL margin% is within (-2.7, +2.7) and statewide %in <= 75,
        # reduce the *higher* adjusted win prob by 15 points (floor 50/50).
        median_gate_penalty_pts = 0.0
        med = bx.get("med", np.nan)

        if np.isfinite(med) and (-2.7 < med < 2.7) and (x <= 75.0):
            median_gate_penalty_pts = 15.0
            if trump_adj >= harris_adj:
                hi_new = max(50.0, trump_adj - median_gate_penalty_pts)
                trump_adj = hi_new
                harris_adj = 100.0 - hi_new
            else:
                hi_new = max(50.0, harris_adj - median_gate_penalty_pts)
                harris_adj = hi_new
                trump_adj = 100.0 - hi_new
