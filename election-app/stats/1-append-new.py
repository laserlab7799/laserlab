...
with open(IN1, "r", newline="", encoding="utf-8") as f1, open(OUT, "w", newline="", encoding="utf-8") as fo:
    ...
    for r in r1:
        ...
        if final_cols is None:
            ...
            # Always carry over identifiers from the snapshot row
            final_cols = {
                "final_state":  r.get("state", ""),
                "final_county": r.get("county", ""),
                "final_fips":   r.get("fips", ""),
                # Only put 10s in vote fields if there is a mismatch
                "final_trump_votes":  "10" if MISMATCH else "",
                "final_harris_votes": "10" if MISMATCH else "",
                "final_other_votes":  "10" if MISMATCH else "",
            }
