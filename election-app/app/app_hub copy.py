...
def _export_stats_once():
    ...
    if office == "H":
        stfp = USPS_TO_STATEFP.get(usps, "")
        district_id = f"{stfp}{STATS_EXPORT_DISTRICT_NUM}" if stfp else STATS_EXPORT_DISTRICT_NUM
...
def _system_usage():
    """Return dict with app + system + (if running) pipeline usage + peak pipeline RSS."""
    global _pipeline_peak_rss_mb

    mem = psutil.virtual_memory()
    out = {
        "cpu_percent": psutil.cpu_percent(interval=None),   # whole system
        "mem_percent": mem.percent,                         # whole system
        "app_rss_mb": psutil.Process(os.getpid()).memory_info().rss / (1024 * 1024),
    }

    # default peak (in case it's not defined yet elsewhere)
    try:
        _pipeline_peak_rss_mb
    except NameError:
        _pipeline_peak_rss_mb = 0.0

    # If pipeline is running, include its RSS/CPU too
    try:
        if _pipeline_proc is not None and _pipeline_proc.poll() is None:
            p = psutil.Process(_pipeline_proc.pid)
            rss_mb = p.memory_info().rss / (1024 * 1024)
            _pipeline_peak_rss_mb = max(_pipeline_peak_rss_mb, rss_mb)

            out["pipeline_rss_mb"] = rss_mb
            out["pipeline_peak_rss_mb"] = _pipeline_peak_rss_mb
            out["pipeline_cpu_percent"] = p.cpu_percent(interval=None)
        else:
            out["pipeline_rss_mb"] = 0.0
            out["pipeline_peak_rss_mb"] = _pipeline_peak_rss_mb
            out["pipeline_cpu_percent"] = 0.0
    except Exception:
        out["pipeline_rss_mb"] = None
        out["pipeline_peak_rss_mb"] = _pipeline_peak_rss_mb
        out["pipeline_cpu_percent"] = None

    return out



def _kick_pipeline_after_export():
    """Run: python pipeline.py --statewide-input stats.csv --stats2-input stats2.csv (cwd=STATS_DIR)
       Then trigger convert_plotdata_folder.py one level up."""
    global _pipeline_proc
    if not STATS_EXPORT_RUN_PIPELINE:
        return

    with _pipeline_lock:
        # Avoid duplicate concurrent pipelines
        if _pipeline_proc is not None and _pipeline_proc.poll() is None:
            return

        try:
            cmd = [
                sys.executable, PIPELINE_CMD,
                "--statewide-input", "stats.csv",
                "--stats2-input", "stats2.csv",
            ]
            _pipeline_proc = subprocess.Popen(cmd, cwd=STATS_DIR)
            log(f"Pipeline started: {' '.join(cmd)} (cwd={STATS_DIR})")

            # ---- NEW: wait for pipeline to finish, then run converter ----
            def _wait_and_convert():
                _pipeline_proc.wait()
                log("Pipeline finished, running convert_plotdata_folder.py…")
                try:
                    win_prob_dir = os.path.join(PARENT_DIR, "win_prob")
                    converter = [
                        sys.executable,
                    ...
                    subprocess.run(converter, cwd=win_prob_dir, check=False)
                    log(f"convert_plotdata_folder.py completed for {STATS_EXPORT_USPS} {STATS_EXPORT_YEAR} (cwd={win_prob_dir})")
                except Exception as e:
                    log(f"Converter run failed: {e}", "WARN")


            threading.Thread(target=_wait_and_convert, daemon=True).start()

        except Exception as e:
            log(f"Pipeline start failed: {e}", "WARN")
...
def _hub_loop():
    ...
    while True:
        ...
        try:
            u = _system_usage()
            log(
                f"System CPU={u['cpu_percent']:.1f}% | RAM={u['mem_percent']:.1f}% | "
                f"app={u['app_rss_mb']:.0f}MB | "
                f"pipeline={u.get('pipeline_rss_mb',0):.0f}MB (peak {u.get('pipeline_peak_rss_mb',0):.0f}MB)"
            )
...
def admin_system_usage():
    try:
        cpu, mem_used, mem_pct = _system_usage()
        return jsonify({
            "cpu_percent": cpu,
