    ...
    while True:
        try:
            if _hub_cfg["mode"] == "general":
                # lock the runtime full URL to the configured full_date
                full_date = _hub_cfg.get("full_date") or "2024-11-05"
                base = f"{BASE_URL_E}/{full_date}?level={LEVEL_PARAM}&resultstype=b"
                globals()["CURRENT_FULL_DATA_URL"] = base
                ok = _fetch_full_once()
