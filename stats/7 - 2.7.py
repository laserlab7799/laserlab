...
def main():
    ...
    for ts in targets:
        ...
        # --- NEW: "2.7 median gate" penalty (on top of the others) ---
        # If statewide%in < 75 AND MC median margin% is in [-2.7, +2.7],
        # subtract 15 points from whichever candidate currently has the higher ADJUSTED win prob.
        # Floor at 50-50 and keep probabilities summing to 100.
        median_gate_penalty_pts = 0.0
        mc_med = float(bx.get("med", np.nan))

        if (x < 75.0) and np.isfinite(mc_med) and (-2.7 <= mc_med <= 2.7):
            if trump_adj >= harris_adj:
                before = trump_adj
                trump_adj = max(50.0, trump_adj - 15.0)
                harris_adj = 100.0 - trump_adj
                median_gate_penalty_pts = before - trump_adj
            else:
                before = harris_adj
                harris_adj = max(50.0, harris_adj - 15.0)
